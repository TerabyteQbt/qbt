...
public final class UpdatePackage extends QbtCommand<UpdatePackage.Options> {
    ...
    public int run(final OptionsResults<? extends Options> options) throws IOException {
        ...
        ImmutableList<String> qbtEnvToRemove = options.get(Options.removeQbtEnv);
        if(!qbtEnvToRemove.isEmpty()) {
            pmd = pmd.transform(PackageMetadata.QBT_ENV, (envMap) -> ImmutableMap.<String, Maybe<String>>builder().putAll(Maps.filterEntries(envMap, (input) -> !qbtEnvToRemove.contains(input.getKey()))).build());
        }
